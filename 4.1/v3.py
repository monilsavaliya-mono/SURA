...
def precompute_V_parallel():
    ...
    nw = cpu_count()
    chunksize = max(1, len(jobs) // (nw * 4))
    report_every = max(1, len(jobs) // 20)
    print(f"FK on {len(jobs)} nodes across {nw} cores ...")
    with Pool(nw) as pool:
        done = 0
        for i, j, v in pool.imap_unordered(_fk_node, jobs, chunksize=chunksize):
            Vgrid[i, j] = v
            done += 1
...
def optimal_dose(nS, nR, c_floor=0.0):  
    ...
    H = (nS * (_k2_scan[mask]  * g_S - _kl_k2[mask])
   + nR * (_k4r_scan[mask] * g_R - _kl_k4r[mask])
   - w_C * (_c_scan[mask] / micS) ** 2)
...
def build_dose_map_parallel(DS, DR, cap=10.0):
    ...
    chunksize = max(1, len(jobs) // (nw * 4))
    with Pool(nw) as pool:
        for ns, nr, c in pool.imap_unordered(_dose_node, jobs, chunksize=chunksize):
...
def build_dose_slices_parallel(nS_scan, nR_slices):
    ...
    chunksize = max(1, len(jobs) // (nw * 4))
    with Pool(nw) as pool:
        for ns, nr, c in pool.imap_unordered(_dose_slice_node, jobs, chunksize=chunksize):
