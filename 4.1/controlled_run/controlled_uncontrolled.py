...
w_T   = 0.0      # time penalty per hour
w_N   = 1.0      # load penalty (÷K normalised)
w_Rr  = 100.0    # running resistance penalty (÷K normalised)
...
def optimal_dose(nS, nR):
    ...
    v_here = V_at(nS, nR)
    v_down = V_at(nS - 1, nR)
    if not (np.isfinite(v_here) and np.isfinite(v_down)):
        return 0.0
    dV     = np.clip(v_down - v_here, -20, 20)
    k2_opt = (k4 + _aS_base) * np.exp(dV)
    alpha  = k2_opt - k4
    if not np.isfinite(alpha) or alpha <= 0:
        return 0.0
    return invert_alpha_S(alpha)
